...
// Defines the options accepted by adminAuthedFetch, inherited from the native RequestInit.
...
/**
 * A React Hook that encapsulates the admin authentication process,
 * providing a fetch function with automatic token refresh and retry mechanism.
 * 邏輯跟 student 端的 useAuthFetch 完全一致，只是吃 AdminAuthContext 的
 * token/refresh/restore，打的也是 admin 專屬的 refresh 流程。
 * @returns { adminAuthedFetch: (url: string, options?: AuthFetchOptions) => Promise<any> }
 */
export const useAdminAuthFetch = () => {
  const {
    tokenRef,
    accessToken,
    refreshAccessToken,
  ...
  const adminAuthedFetch = useCallback(
    async (url: string, options: AuthFetchOptions = {}) => {
      const { retries = 1, bypass = false, ...fetchOptions } = options;
      const maxAttempts = retries + 1;
      let lastError: Error | null = null;

      if (!activeAdminRef.current && restorePromise) {
        // If `activeAdmin` has not been set (App just launch)，
        // and `AdminAuthContext` has waiting `restorePromise，`
        // then wait for the session to resume and the Promise to complete.
        console.log(
          "[AdminAuthFetch] Waiting for session restore to complete..."
        );
        const result = await restorePromise;

        // After waiting, if `activeAdmin` is still `null`
        // it failed.
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        try {
          ...
          if (res.status === 401) {
            console.warn(
              `[AdminAuthFetch] Attempt ${attempt}: Recieved ${res.status} error, trying to refresh Access Token...`
            );
            ...
            try {
              ...
              console.log(
                `[AdminAuthFetch] Token refresh succeeded, ${
                  attempt + 1
              ...
              console.error(
                "[AdminAuthFetch] Failed to refresh Access Token, re-login is requeired.",
                refreshError
              );
          ...
          console.error(`[AdminAuthFetch] Attempt ${attempt} failed:`, error);
